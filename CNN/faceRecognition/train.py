...
def train(model, datas, device, learning_rate=0.0005):
    model.train()  # 训练模式开启
    triple_loss_fn = torch.nn.TripletMarginLoss(margin=100.0, p=2)
    ...
    for i, data in enumerate(datas, 0):
        ...
        if i % 10 == 9:
            print("No.%03d loss: %.6f" % (i+1, float(loss)))
...
if __name__ == "__main__":
    ...
    try:
        ...
        model_in = dir + '/' + fileName
        model_out = dir + "/model_%03d.pth" % (int(fileName.split('.')[0].split('_')[1])+1)
    except:
        model_in = None
    ...
    print("开始加载数据。。。")
    train_loader = getFaceDatas.getTrainDataSet()
    print("开始训练模型。。。")
